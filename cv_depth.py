...
def run_cross_validation_for_model(data, depths, number_of_folds, model_idx):
    ...
    for d in depths:
        ...
        for i in range(number_of_folds):
            ...
            training_ac, test_ac = model_func(train_set, test_set, depth=d)
        ...
        write_model_results_to_file('d_' + get_model_name(model_idx) + '_' + str(d), 'tr', fold_tr_ac, [])
        write_model_results_to_file('d_' + get_model_name(model_idx) + '_' + str(d), 'ts', fold_ts_ac, [])
...
if __name__ == "__main__":
    ...
    for model_idx in model_idxs:
        avg_ac_tfrac, se_tfrac = run_cross_validation_for_model(data, depths, number_of_folds, model_idx)
        for x in avg_ac_tfrac:
            write_model_results_to_file('d_' + get_model_name(model_idx), x, avg_ac_tfrac[x], se_tfrac[x])
    ...
    for model_idx in model_idxs:
        sets = ['ts']
        for s in sets:
            avg_ac, se = read_model_results_from_file('d_' + get_model_name(model_idx), s)
            plt.errorbar(depths, avg_ac, yerr=se, label=get_model_name(model_idx) + '_' + s + '_ac', fmt='o-')
    ...
    plt.savefig('outputs/' + '3_a.pdf', format='pdf')
    plt.clf()

    # Answer to the question 3.b
    test_hypothesis(read_results_for_frac('outputs/d_' + get_model_name(3), 'ts', 7),
                    read_results_for_frac('outputs/d_' + get_model_name(2), 'ts', 7)
                    )
